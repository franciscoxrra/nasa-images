...
type HistoryAction<
    Type,
    Payload = undefined
> = PrefixAction<"history", Type, Payload>

export type AddToHistory = HistoryAction<
    "history/add",
    HistoryEntry
>
...
const isDuplicatedEntry = (
    state: HistoryState,
    action: AddToHistory
) => {
    if (state.previousSearchExpressions.length > 0) {
        const lastSearchExpressionData =
            state.previousSearchExpressions[0]
        if (
            action.payload.expression ===
            lastSearchExpressionData.expression
        ) {
            const existingDate = new Date(
                lastSearchExpressionData.lastViewDate
            )
            const newDate = new Date(
                action.payload.lastViewDate
            )
            if (
                newDate.getTime() - existingDate.getTime() <
                    searchDuplicateDebounceMs &&
                lastSearchExpressionData.lastViewPage ===
                    action.payload.lastViewPage
            ) {
                Sentry.withScope((scope) => {
                    ...
                    const err = new Error(
                        `CAUGHT HISTORY ENTRY DUPLICATION`
                    )
    ...
    switch (action.type) {
        case "history/add":
            return action.payload &&
                !isDuplicatedEntry(state, action)
                ? isPageChange(state, action)
                    ? {
                          ...state,
                          previousSearchExpressions: [
                              action.payload,
                              ...state.previousSearchExpressions.slice(
                                  1,
                                  5
                              )
                          ]
